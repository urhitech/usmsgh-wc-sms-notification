...
class WeDevs_Settings_API {
    ...
    function show_forms() {
			...
			<?php foreach ( $this->settings_sections as $form ) { ?>
                <div id="<?php echo esc_attr($form['id']); ?>" class="group" style="display: none;">
                    <?php if( array_key_exists('children', $form) ) { ?>
                        ...
                        <?php } else { ?>
                            <h2>Oops, looks like you did not have any plugins that we support. See the full supported plugins for automations <a href="https://wordpress.org/plugins/usmsgh-wc-sms-notification/">here</a></h2>
                        <?php } ?>
                    <?php } else { ?>
                        <?php
                        ...
                        <form id="<?php echo $form['id'] . "_form" ?>" method="post" action="<?php echo esc_url($action_url); ?>">
                            ...
                            do_action('usmsgh_setting_fields_custom_html', $form['id']);
        ...
        <?php
        $this->script();
        do_action('usmsgh_load_javascripts');
    }
