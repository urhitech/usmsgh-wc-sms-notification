...
class Usmsapi_Multivendor_WooCommerce_Product_Vendors_Manager extends Abstract_Usmsgh_Multivendor {
	...
	public function setup_mobile_number_setting_field( $user ) {
		?>
        <h3 class="heading">UsmsGH-API Woocommerce</h3>
            ...
            <tr>
                ...
                <td>
                    <input type="text" class="input-text" id="usmsgh_phone_field" name="usmsgh_phone_field"
                           value="<?php echo esc_attr( get_the_author_meta( 'usmsgh_phone', $user->ID ) ) ?>"/>
	...
	public function get_vendor_country_from_vendor_data($vendor_data){
		$selected_country_code        = usmsgh_get_options( 'usmsgh_woocommerce_country_code', 'usmsgh_setting', '' );//Get default country v1.1.17
	...
	public function get_vendor_data_list_from_order( $order_id ) {
		...
		$this->log->add( 'UsmsGH', 'Raw data: ' . json_encode( $vendor_data_list ) );
