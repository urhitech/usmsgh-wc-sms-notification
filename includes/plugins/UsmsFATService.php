...
class UsmsFATService implements Usmsgh_PluginInterface, Usmsgh_Register_Interface {
    /*
    ...
    */
    ...
    private function get_sms_template_fields() {
        return array(
            array(
                'name'    => 'usmsgh_automation_sms_template_cancel',
                'label'   => __( 'Cancel SMS message', USMSGH_TEXT_DOMAIN ),
                'desc'    => sprintf('Customize your SMS with <button type="button" id="usmsgh-open-keyword-%1$s-[dummy]" data-attr-type="cancel" data-attr-target="%1$s[usmsgh_automation_sms_template_cancel]" class="button button-secondary">Keywords</button>', $this->get_option_id() ),
                'type'    => 'textarea',
                'rows'    => '8',
                'cols'    => '500',
                'css'     => 'min-width:350px;',
                'default' => __( 'Greetings [c_first_name], your appointment for [s_name] on [b_date] [b_time] is [b_process_status]', USMSGH_TEXT_DOMAIN )
            ),
            array(
                'name'    => 'usmsgh_automation_sms_template_approved',
                'label'   => __( 'Approved SMS message', USMSGH_TEXT_DOMAIN ),
                'desc'    => sprintf('Customize your SMS with <button type="button" id="usmsgh-open-keyword-%1$s-[dummy]" data-attr-type="approved" data-attr-target="%1$s[usmsgh_automation_sms_template_approved]" class="button button-secondary">Keywords</button>', $this->get_option_id() ),
                'type'    => 'textarea',
                'rows'    => '8',
                'cols'    => '500',
                'css'     => 'min-width:350px;',
            ...
            array(
                'name'    => 'usmsgh_automation_sms_template_reject',
                'label'   => __( 'Rejected SMS message', USMSGH_TEXT_DOMAIN ),
                'desc'    => sprintf('Customize your SMS with <button type="button" id="usmsgh-open-keyword-%1$s-[dummy]" data-attr-type="approved" data-attr-target="%1$s[usmsgh_automation_sms_template_reject]" class="button button-secondary">Keywords</button>', $this->get_option_id() ),
                'type'    => 'textarea',
                'rows'    => '8',
                'cols'    => '500',
                'css'     => 'min-width:350px;',
    ...
    public function get_plugin_settings($with_identifier = false)
    {
        $settings = array(
            ...
            "usmsgh_automation_sms_template_cancel"   => usmsgh_get_options("usmsgh_automation_sms_template_cancel", $this->get_option_id()),
            "usmsgh_automation_sms_template_approved" => usmsgh_get_options("usmsgh_automation_sms_template_approved", $this->get_option_id()),
            "usmsgh_automation_sms_template_pending"  => usmsgh_get_options("usmsgh_automation_sms_template_pending", $this->get_option_id()),
            "usmsgh_automation_sms_template_reject"   => usmsgh_get_options("usmsgh_automation_sms_template_reject", $this->get_option_id()),
        );
    ...
    public function send_sms_on($b_id, $b_process_status)
    {
        ...
        $this->log->add("UsmsGH", "booking id: {$b_id}");
        $this->log->add("UsmsGH", "booking status: {$status}");

        if($enable_notifications === "on"){
            $this->log->add("UsmsGH", "Enable notifications: {$enable_notifications}");
            if(!empty($send_on) && is_array($send_on)) {
                if(array_key_exists($status, $send_on)) {
                    $this->log->add("UsmsGH", "Enable {$status} notifications: true");
                    $booking = $this->get_booking_by_id($b_id);
                    // if booking is empty, nothing to do here
                    if(empty($booking)) {
                        $this->log->add("UsmsGH", "booking is empty, nothing else to do. Aborting");
                        return;
                    }
    ...
    public function send_customer_notification($booking, $status)
    {
        ...
        if(empty($booking->c_phone_code) && empty($booking->c_phone)) {
            $this->log->add("UsmsGH", "customer country code and phone is empty");
            return;
        }

        $country_code = explode(',', $booking->c_phone_code)[1];
        $phone_no = UsmsGH_SendSMS_Sms::get_formatted_number($booking->c_phone, $country_code);

        if(empty($phone_no)) {
            $this->log->add("UsmsGH", "Number invalid format. Aborting");
            return;
        }

        // $phone_no = "{$country_code}{$booking->c_phone}";
        $this->log->add("UsmsGH", "customer phone no: {$phone_no}");

        // get message template from status
        $msg_template = $settings["usmsgh_automation_sms_template_{$status}"];

        $this->log->add("UsmsGH", "Message template: {$msg_template}");
