...
namespace UsmsAPI_WC\Forms\Handlers;

use UsmsAPI_WC\Helpers\Sanitization;
use \Usmsgh_WooCoommerce_Logger;
...
class ContactForm7 {
    ...
    public function __construct() {
        $this->_option_prefix = 'usmsapi_sms_wpcf7_';
		...
		add_filter( 'wpcf7_validate_tel*', array( $this, 'validateFormPost' ), 1, 2 );
		add_filter( 'wpcf7_validate_usms_phone', array( $this, 'validateFormPost' ), 10, 2 );
		add_filter( 'wpcf7_validate_usms_phone*', array( $this, 'validateFormPost' ), 10, 2 );
		add_filter( 'wpcf7_messages', array( $this, 'wpcf7_usms_phone_messages' ), 10, 1 );

		add_filter( 'wpcf7_editor_panels', array( $this, 'new_menu_usms' ), 10, 1 );

		add_action( 'wpcf7_admin_init', array( $this, 'add_usmsapi_phone_tag' ), 20, 0 );
        add_action( 'wpcf7_after_save', array( &$this, 'save_form' ) );
		add_action( 'wpcf7_before_send_mail', array( $this, 'sendsms_c7' ) );

		add_action( 'wpcf7_init', array( $this, 'usmsapi_wpcf7_add_shortcode_phonefield_frontend' ) );
		add_action( 'wpcf7_admin_notices', array( $this,'usmsapi_wpcf7_show_warnings'), 10, 3 );
    ...
    public function save_form($form) {
        // identifier = usmsapi_sms_wpcf7_{id}
        /* array (
        ...
        */
        $wpcf7usmsapi_settings = ( ! empty( $_POST['wpcf7usmsapi-settings'] ) ) ? wp_unslash( $_POST['wpcf7usmsapi-settings'] ) : '';
		update_option( $this->_option_prefix . $this->get_contact_form_id($form), Sanitization::usmsapi_sanitize_array( $wpcf7usmsapi_settings ) );
    }
	...
	public function sendsms_c7( $form ) {
        $this->log->add("UsmsGH", "Initiating Send SMS c7");
		...
		if ( $send_to_admin ) {
            $this->log->add("UsmsGH", "Sending SMS c7 to admin");

            foreach ($admin_numbers as $admin_number) {
                $validated_admin_number = \UsmsGH_SendSMS_Sms::get_formatted_number($admin_number);
                \UsmsGH_SendSMS_Sms::send_sms('', $validated_admin_number, $admin_message);
            }
		}

		if ( $send_to_visitor ) {
            $this->log->add("UsmsGH", "Sending SMS c7 to visitor");
            $validated_visitor_number = \UsmsGH_SendSMS_Sms::get_formatted_number($visitor_number);
            \UsmsGH_SendSMS_Sms::send_sms('', $validated_visitor_number, $visitor_message);
		}
    ...
    public function new_menu_usms( $panels ) {
		$panels['usms-sms-panel'] = array(
			'title'    => __( 'UsmsGH-API' ),
			'callback' => array( $this, 'add_panel_usms' ),
		);
		return $panels;
	}
	 ...
	 * @return void
	 */
	public function usmsapi_wpcf7_add_shortcode_phonefield_frontend() {
		wpcf7_add_form_tag(
			array( 'usms_phone', 'usms_phone*'),
			array( $this, 'usmsapi_wpcf7_shortcode_handler' ),
			true
		);
	}
	 ...
	 * @return void
	 */
	public function add_panel_usms( $form ) {
		if ( wpcf7_admin_has_edit_cap() ) {
			...
			$options['_form'] = $form;
			$data            = $options;
			include USMSGH_PLUGIN_DIR . '/src/Forms/Views/ContactForm7View.php';
		}
	}

    public function add_usmsapi_phone_tag()
    {
        if ( class_exists( 'WPCF7_TagGenerator' ) ) {
			$tag_generator = \WPCF7_TagGenerator::get_instance();
			$tag_generator->add( 'usms_phone', __( 'USMSAPI PHONE', 'contact-form-7' ), array( $this, 'usmsapi_wpcf7_tag_generator_text' ) );
		}
    }

        /**
	 * Handle usms wpcf7 shortcode.
	 *
	 * @param  object $tag get tag objects.
	 *
	 * @return string
	 */
	public function usmsapi_wpcf7_shortcode_handler( $tag ) {
		...
		$class = wpcf7_form_controls_class( $tag->type, 'wpcf7-usmsapi' );
		if ( $validation_error ) {
    	...
    	/**
	 * Tag generator form for usmsapi phone tag in cf7 backend
	 *
	 ...
	 * @return void
	 */
	public function usmsapi_wpcf7_tag_generator_text( $contact_form, $args = '' ) {
		$args = wp_parse_args( $args, array() );
		$type = $args['id'];
        $field_name = 'usms_phone';
		?>
    ...
    /**
	 * Validate form post for usms_phone field at frontend.
	 *
	...
	public function validateFormPost( $result, $tag ) {
		...
		if ( in_array( $tag->basetype, array( 'usms_phone' ), true ) ) {
            if($tag->is_required()) {
                if(empty( $value )) {
                    $result->invalidate( $tag, wpcf7_get_message( 'invalid_required' ) );
                } else {
                    if ( ! \UsmsGH_SendSMS_Sms::get_formatted_number( $value ) ) {
                        $result->invalidate( $tag, wpcf7_get_message( 'usms_invalid_number' ) );
                    }
                }
            } else  {
                if(!empty( $value ) && ! \UsmsGH_SendSMS_Sms::get_formatted_number( $value ) ) {
                    $result->invalidate( $tag, wpcf7_get_message( 'usms_invalid_number' ) );
                }
            }
	 ...
	 * @return object
	 */
	public function wpcf7_usms_phone_messages( $messages ) {
		return array_merge(
			$messages,
			array(
				'usms_invalid_number' => array(
					'description' => __( 'Invalid number', USMSGH_TEXT_DOMAIN ),
					'default'     => __( 'Invalid number', USMSGH_TEXT_DOMAIN ),
				),
			)
		);
	}
    ...
    /**
	 * Show warning if usms_phone phone field not selected.
	 *
	 ...
	 * @return void
	 */
	function usmsapi_wpcf7_show_warnings($page,$action,$object)
	{
		...
		if(!empty($_REQUEST['post'])){
			...
			if ( empty($options['visitor_mobile_field']) )
			{
				echo sprintf(
					...
					esc_html__( "Please choose mobile number field in UsmsGH-API tab", USMSGH_TEXT_DOMAIN)
				);
			}
		}
	}
